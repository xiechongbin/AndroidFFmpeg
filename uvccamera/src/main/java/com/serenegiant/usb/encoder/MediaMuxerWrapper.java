...
public class MediaMuxerWrapper {
    ...
    private static final String DIR_NAME = "appplatform";
     ...
     * @param path
     * @param ext
     * @param outSidePath 是否是外部传入视频文件路径
     * @throws IOException
     */
    public MediaMuxerWrapper(String path, String ext, boolean outSidePath) throws IOException {
        if (TextUtils.isEmpty(ext)) ext = ".mp4";
        if (outSidePath) {
            mOutputPath = path;
        } else {
            try {
                mOutputPath = getCaptureFile(path, Environment.DIRECTORY_MOVIES, ext).toString();
            } catch (final NullPointerException e) {
                throw new RuntimeException("This app has no permission of writing external storage");
            }
        }
...
//**********************************************************************
    ...
    public static final File getCaptureFile(final String path, final String type, final String ext) {
